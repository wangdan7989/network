...
/**
 * Walk snmp value to the structure my_oid_result.
 ...
 * @return void.
 */
 //ÔõÃ´µÃµ½netsnmp_variable_list *vars
...
void my_snmp_bulkwalk_perror(unsigned int ratval) {
	switch (ratval) {
		case 1:
			printf("[ERROR] my_snmp_bulkwalk: the argument value oid_names is empty\n");
			break;
		case 2:
			printf("[ERROR] my_snmp_bulkwalk: the snmp_open function run failed\n");
			break;
		case 3:
			printf("[ERROR] my_snmp_bulkwalk: the snmp_parse_oid function run failed\n");
			break;
		case 4:
			printf("[ERROR] my_snmp_bulkwalk: time out\n");
			break;	
		case 5:
			printf("[ERROR] my_snmp_bulkwalk: snmpget sesson_handls error\n");
			break;
		case 6:
			printf("[ERROR] my_snmp_bulkwalk: end of MIB\n");
			break;
		case 7:
			printf("[ERROR] my_snmp_bulkwalk: response->errstat others error\n");
			break;
		default :
			printf("[ERROR] my_snmp_bulkwalk: no %d error type return value\n", ratval);
	}
}
...
int my_snmp_bulkwalk(const char *peername, 
  	...
  	if(peername == NULL) {
		printf("[ERROR] my_snmp_bulkwalk: the peername pointer is null\n");
		return 1;
	}
	
	if(community == NULL) {
		printf("[ERROR] my_snmp_bulkwalk: the community pointer is null\n");
		return 1;
	}
	
	if(oid_argv == NULL) {
		printf("[ERROR] my_snmp_bulkwalk: the oid_argv pointer is null\n");	
		return 1;
	}

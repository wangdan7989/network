...
int get_ArpDyn_table_info_from_device_by_snmp(const unsigned char *ip, 
	...
	*ip_arpdyn_table_rows = 0;
	...
	/*get the hwArpDynIfIndex,hwArpDynVrf,ip_addr */
	...
	/*get the hwArpDynIfIdex */
			...
			ip_arpdyn_table_info[ip_arpdyn_table_nums].hwArpDynIfIndex=indexip;
	...
	/*get the hwArpDynVrf */		
			...
			ip_arpdyn_table_info[ip_arpdyn_table_nums].hwArpDynVrf=indexvrf;
		...
		for(count = 0; count < oid_results_nums; count++) {
			...
			if(oid_results[count].type == ASN_OCTET_STR || oid_results[count].type == ASN_BIT_STR) {
				...
				if(!hex) {					
					...
					strncpy(ip_arpdyn_table_info[ip_arpdyn_table_nums++].hwArpDynMacAdd, mstr, 19);
				}
			}
	...
	if(my_snmp_bulkwalk(ip, comm, Arp_Dyn_VlanId, oid_results, &oid_results_nums) == 0) {
		...
		for(count = 0; count < oid_results_nums; count++) {	
			if(oid_results[count].type == ASN_INTEGER) {
				ip_arpdyn_table_info[ip_arpdyn_table_nums++].hwArpDynVlanId = oid_results[count].val.integer;
			} else {
	...
	if(my_snmp_bulkwalk(ip, comm, Arp_Dyn_OutIfIndex, oid_results, &oid_results_nums) == 0) {
		...
		for(count = 0; count < oid_results_nums; count++) {	
			if(oid_results[count].type == ASN_INTEGER) {
				ip_arpdyn_table_info[ip_arpdyn_table_nums++].hwArpDynOutIfIndex = oid_results[count].val.integer;
			} else {
				printf("[ERROR] call my_snmp_bulkwalk function to get IP address table if index type is error\n");
				free(oid_results);
				return 1;
...
/* get the Arp_Dyn_ExpireTime   */
	if(my_snmp_bulkwalk(ip, comm, Arp_Dyn_ExpireTime, oid_results, &oid_results_nums) == 0) {
		...
		for(count = 0; count < oid_results_nums; count++) {	
			if(oid_results[count].type == ASN_INTEGER) {
				ip_arpdyn_table_info[ip_arpdyn_table_nums++].hwArpDynExpireTime = oid_results[count].val.integer;
			} else {
				printf("[ERROR] call my_snmp_bulkwalk function to get IP address table if index type is error\n");
				free(oid_results);
				return 1;

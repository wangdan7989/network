#include "/home/wd/workspace/network/common/my_snmp_walk.h"
/**
...
/**
 * Walk snmp value to the structure my_oid_result array.
 * Author : lining 15810423651 lining01@iie.ac.cn 
 ...
 */
int my_snmp_walk (const char *peername, 
	...
	*oid_results_nums = 0;
	...
	netsnmp_ds_register_config(ASN_BOOLEAN, "snmpwalk", "excludeRequested", NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_WALK_DONT_GET_REQUESTED);
	...
	netsnmp_ds_register_config(ASN_BOOLEAN, "snmpwalk", "timeResults", NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_WALK_TIME_RESULTS);
	netsnmp_ds_register_config(ASN_BOOLEAN, "snmpwalk", "timeResultsSingle", NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_WALK_TIME_RESULTS_SINGLE);
	...
	if (snmp_parse_oid(oid_argv, root, &rootlen) == NULL) {
		printf("[ERROR] my_snmp_walk: call snmp_parse_oid function failed\n");
		return 3;
	}
	...
	if (ss == NULL) {
		printf("[ERROR] my_snmp_walk: call snmp_open function failed\n");
		SOCK_CLEANUP;
		return 2;
	}
		...
		if(status == STAT_SUCCESS) {
			...
			if(response->errstat == SNMP_ERR_NOERROR) {
				...
				for(vars = response->variables; vars; vars = vars->next_variable) {
					...
					get_walk_oid_values(vars, &oid_results[numprinted]);
					numprinted++;
				...
				if (response->errstat == SNMP_ERR_NOSUCHNAME ) {
					printf("[ERROR] my_snmp_walk: have reached to the end of MIB file\n");
					return 6;
				} else{
					printf("[ERROR] my_snmp_walk: return response->errstat others error\n");
					return 7;
				}
			}
		} else if(status == STAT_TIMEOUT) {	
			printf("[ERROR] my_snmp_walk: return status is session time out\n");
			running = 0;
			return 4;
		} else {
			printf("[ERROR] my_snmp_walk: return session response error\n");
            running = 0;
			return 5;
        }
...
int main(int argc, char *argv[]){
	...
	int ral = my_snmp_walk(peername, community, oid_name, oid_results, &oid_results_nums);
